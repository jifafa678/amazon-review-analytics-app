...
MIN_BRAND_REVIEW_COUNT = 5
MIN_BRAND_PRODUCT_COUNT = 2
...
@st.cache_data(show_spinner=False)
def get_brand_filter_options() -> tuple[list[str], dict[str, list[str]]]:
    brand_stats = query_data("""
    SELECT
    ...
    brand_stats["clean_brand"] = brand_stats["raw_brand"].apply(clean_brand_name)
    brand_stats = brand_stats[brand_stats["clean_brand"] != "Unknown"].copy()
    brand_stats = brand_stats[
        (brand_stats["review_count"] >= MIN_BRAND_REVIEW_COUNT)
        | (brand_stats["product_count"] >= MIN_BRAND_PRODUCT_COUNT)
    ]

    if brand_stats.empty:
        return ["全部品牌"], {}

    grouped = brand_stats.groupby("clean_brand", as_index=False).agg(
        review_count=("review_count", "sum"),
        product_count=("product_count", "sum"),
        raw_brands=("raw_brand", lambda values: sorted({str(value) for value in values if str(value).strip()})),
    )
    grouped = grouped.sort_values(["review_count", "product_count", "clean_brand"], ascending=[False, False, True])

    options = ["全部品牌"] + grouped["clean_brand"].tolist()
    raw_brand_map = dict(zip(grouped["clean_brand"], grouped["raw_brands"]))
    return options, raw_brand_map
...
def prepare_product_display(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "brand" in x:
        x["brand"] = x["brand"].apply(clean_brand_name)
...
def prepare_brand_display(df: pd.DataFrame) -> pd.DataFrame:
    x = df.copy()
    if "brand" in x:
        x["brand"] = x["brand"].apply(clean_brand_name)
    if "negative_review_ratio" in x:
        x["negative_review_ratio"] = (x["negative_review_ratio"] * 100).round(1)
    if "verified_purchase_ratio" in x:
        x["verified_purchase_ratio"] = (x["verified_purchase_ratio"] * 100).round(1)
    return cn_columns(x)


def clean_brand_rows(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty or "brand" not in df.columns:
        return df.copy()
    x = df.copy()
    x["brand"] = x["brand"].apply(clean_brand_name)
    return x[x["brand"] != "Unknown"].copy()
...
"""))
brand_all = clean_brand_rows(brand_all)
...
brand_options, raw_brand_map = get_brand_filter_options()
...
with tab1:
    ...
    if selected_brand and selected_brand != "全部品牌":
        raw_brand_values = raw_brand_map.get(selected_brand, [selected_brand])
        placeholders = ",".join(["?"] * len(raw_brand_values))
        product_sql += f" AND p.brand IN ({placeholders}) "
        params.extend(raw_brand_values)
    product_sql += """
    GROUP BY p.product_id,p.product_title,p.brand,p.main_category,p.price,p.average_rating,p.rating_number
...
with tab2:
    ...
    """))
    brand_performance = clean_brand_rows(brand_performance)
    left_col, right_col = st.columns([1.15, 1])
    with left_col:
        st.markdown("**评论量较高的品牌**")
        display_dataframe(prepare_brand_display(brand_performance), height=420)
    with right_col:
    ...
    if not brand_performance.empty:
        ...
        fig.update_layout(height=500, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#FFFFFF"), margin=dict(l=20, r=20, t=35, b=35), xaxis=dict(title="品牌评论量", showgrid=True, gridcolor="rgba(0,209,255,.12)", zeroline=False), yaxis=dict(title="差评率", tickformat=".0%", showgrid=True, gridcolor="rgba(0,209,255,.12)", zeroline=False), legend=dict(title=dict(text="四象限类别", font=dict(color="#FFFFFF")), font=dict(color="#FFFFFF")))
        st.plotly_chart(fig, width="stretch", config={"displayModeBar": False})
        st.markdown('<div class="small-muted" style="color:#FFFFFF;"><b style="color:#FFFFFF;">四象限解读：</b><br>高评论量 + 高差评率：<span style="color:#FFFFFF;font-weight:700;">重点风险品牌</span>；高评论量 + 低差评率：<span style="color:#FFFFFF;font-weight:700;">优势品牌</span>；低评论量 + 高差评率：<span style="color:#FFFFFF;font-weight:700;">潜在风险品牌</span>；低评论量 + 低差评率：<span style="color:#FFFFFF;font-weight:700;">普通观察品牌</span>。</div>', unsafe_allow_html=True)
